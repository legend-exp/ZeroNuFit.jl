...
# Returns
- The sum of background and signal components of the model at position x.
"""
function fit_model(
...
"""
    plot_data(hist::Histogram, name, partitions, part_event_index, pars, samples, posterior, plotflag, settings::Dict, bkg_shape::Symbol, fit_ranges, bkg_units::String) -> Plot
...
# Returns
- A plot object showing data and fit results with excluded gamma line bands.
"""
function plot_data(
    hist::Histogram,
    name,
...
"""
    plot_fit_and_data(partitions, events, part_event_index, samples, posterior, pars, output, config, fit_ranges; toy_idx=nothing) -> Nothing
...
# Returns
- Nothing. Saves a plot to the output folder.
"""
...
"""
    plot_correlation_matrix(samples, output; par_names=nothing, toy_idx=nothing) -> Nothing
...
# Returns
- Nothing. Saves a correlation matrix heatmap to the output folder.
"""
...
"""
    plot_two_dim_posteriors(samples, pars, output; par_names=nothing, toy_idx=nothing) -> Nothing
...
# Returns
- Nothing. Saves 2D posterior plots to a PDF file in the output folder.
"""
...
"""
    plot_marginal_distr(partitions, samples, pars, output; sqrt_prior=false, priors=nothing, par_names=nothing, plot_config=nothing, s_max=nothing, hier=false, toy_idx=nothing) -> Nothing
...
# Returns
- Nothing. Saves marginalized posterior plots to a PDF file in the output folder.
"""
